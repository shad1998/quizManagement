...
if(isset($_POST['submit'])){
	if(!empty($_POST['quizcheck'])){
		...
		$q="select * from question where sub_id='s2'";
...
$name=$login_session;
$total1=$total-200;
$final="insert into result(username,total,correct,sub_id) values('$name','$total1','$result','s2')";
...
<?php
		...
		while($rows=mysqli_fetch_array($query)){
			?>
			<div class="card">
				<h4 class="card-header bg-info text-white"><?php echo ($i%200).". " ?><?php echo  $rows['question'];?> </h4>
