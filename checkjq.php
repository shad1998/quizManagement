...
if(isset($_POST['submit'])){
	if(!empty($_POST['quizcheck'])){
		...
		$q="select * from question where sub_id='s4'";
...
$name=$login_session;
$total1=$total-400;
$final="insert into result(username,total,correct,sub_id) values('$name','$total1','$result','s4')";
...
<?php
		...
		while($rows=mysqli_fetch_array($query)){
			?>
			<div class="card">
				<h4 class="card-header bg-info text-white"><?php echo ($i%400).". " ?><?php echo  $rows['question'];?> </h4>
